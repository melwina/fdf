...
RESULTS_DIR   = "results"
RESULTS_FILE  = os.path.join(RESULTS_DIR, f"{MODEL_NAME[:10]}_binary_results_val.txt")
PRED_CSV      = os.path.join(RESULTS_DIR, f"{MODEL_NAME[:10]}_binary_predictions_val.csv")
...
class ManifestEvalDataset(Dataset):
    """
    Returns (image_tensor, binary_label, source_key, image_path_for_output)
    """
    def __init__(self, df: pd.DataFrame, transform=None, label_col="binary_label", failure_log_path="eval_loader_failures.log"):
...
# ---------------- DataLoader ----------------
eval_ds = ManifestEvalDataset(df, transform=val_transform, label_col="binary_label")
...
pbar = tqdm(total=len(eval_loader.dataset), desc="Evaluating", unit="img")
with torch.inference_mode():
    for batch in eval_loader:
        ...
        yhat_cpu  = logits_cpu.argmax(1)
        t_cpu     = targets.detach().cpu().numpy()

        all_logits.extend(logits_cpu.tolist())   # store logits
        all_preds.extend(yhat_cpu.tolist())      # store preds (class ids)

        all_targets.extend(t_cpu.tolist())
        all_image_paths.extend(list(img_paths))

        # per-source accum
        for sk, y, yhat in zip(source_keys, t_cpu, yhat_cpu):
            st = source_stats.get(sk)
            if st is None:
                st = {"n": 0, "correct": 0, "pred_counts": np.zeros(num_classes, dtype=np.int64)}
                source_stats[sk] = st
            st["n"] += 1
...
with open(RESULTS_FILE, "w") as f:
    ...
    pred_counts = np.bincount(np.array(all_preds), minlength=num_classes) if total else np.zeros(num_classes, dtype=np.int64)
