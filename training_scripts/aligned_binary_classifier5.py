...
TRAIN_TARGET = 2000     # instead of 80_000
VAL_TARGET   = 500      # instead of 20_000
...
early_stopping = EarlyStopping(patience=5, min_delta=1e-8, restore_best_weights=True, verbose=True)
...
def train_model():
    ...
    plt.savefig('../plots/2align_binary_training_curves.png', dpi=300, bbox_inches='tight')
    print("Training curves saved as '../plots/2align_binary_training_curves.png'")

    # ---------------- Save Model ----------------
    model_path = os.path.join(os.getcwd(), 'models', '2align_binary_face_classifier_xception_early_stopped.pth')
    torch.save(model.state_dict(), model_path)
    print(f"\nModel saved as '{model_path}'")
    print("Note: Saved model contains the best weights from the training process")

# Only run training if this script is executed directly, not when imported
