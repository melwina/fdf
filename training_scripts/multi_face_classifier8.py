...
from multi_test_fn import number_of_incorrect_predictions
...
# TRAIN_TARGET = 200   # instead of 80_000
...
class ManifestDataset(Dataset):
    classes = ['deepfake','diffusion','gan','real']  # 0=deepfake, 1=diffusion, 2=gan, 3=real

    def __init__(self, df: pd.DataFrame, transform=None, label_col="multi_label", split_col=None, split_value=None, failure_log_path=FAIL_LOG):
...
# Pools from manifest
train_pool_ds = ManifestDataset(df, transform=train_transform, label_col="multi_label",
                                split_col="train", split_value=1)
test_pool_ds  = ManifestDataset(df, transform=val_transform,   label_col="multi_label",
...
plt.savefig('./plots/8multi_face_classifier_xception_early_stopped_intersection_weighted.png', dpi=300, bbox_inches='tight')
print("Training curves saved as './plots/8multi_face_classifier_xception_early_stopped_intersection_weighted.png'")

# ---------------- Save Model ----------------
model_path = os.path.join(os.getcwd(), 'models', '8multi_face_classifier_xception_early_stopped_intersection_weighted.pth')
