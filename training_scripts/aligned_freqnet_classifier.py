...
# TRAIN_TARGET = 100   # instead of 80_000
...
class FaceClassifier(nn.Module):
    def __init__(self,
        ...
        self.backbone.fc = nn.Sequential(
            nn.Dropout(dropout),
            nn.Linear(num_features, 1)  # Single logit output
        )
...
try:
    ...
    if early_stopping.best_weights is not None:
        print("Restoring best model weights before saving...")
        early_stopping.restore_checkpoint(model)
    else:
        print("No best weights available, saving current model state...")
...
if early_stopping.best_weights is not None:
    print("\nRestoring best model weights for final save...")
    early_stopping.restore_checkpoint(model)
